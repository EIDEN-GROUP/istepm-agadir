...
function DashboardFamilles() {
  ...
  const filtered = useMemo(() => {
    ...
    return clients.filter((c) =>
      `${c.parentName} ${c.childName} ${c.email} ${c.phone}`
        .toLowerCase()
        .includes(q),
    );
  }, [clients, search]);
  ...
  const importMutation = useMutation({
    mutationFn: (csvText: string) =>
      api.post("/clients/import-csv", { csvText }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["clients"] });
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e) => {
              ...
              if (f) {
                ...
                r.onload = () => importMutation.mutate(r.result as string);
                r.readAsText(f);
              }
            }}
          />
          ...
          <button onClick={() => setAddOpen(true)} className={primaryPill}>
            <Plus className="h-4 w-4" /> {t.familles.addClient}
          </button>
        </div>
      </header>
                ...
                >
                  <td
                    ...
                    style={{
                      borderLeftColor:
                        STATUS_COLORS[
                          c.paymentStatus as keyof typeof STATUS_COLORS
                        ] ?? "#ccc",
                    }}
                  >
                    {c.parentName}
