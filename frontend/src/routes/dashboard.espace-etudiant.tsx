      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            {/* Profil */}
            <section className={cn(softCard, "space-y-4 p-5")}>
              <p className={eyebrowClass}>Mon profil</p>
              <div className="flex items-center gap-4">
                <button
                  type="button"
                  onClick={() => photoInput.current?.click()}
                  title="Changer ma photo"
                  className="relative grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-full bg-gradient-to-br from-brand/25 to-brand/10 text-xl font-bold text-brand-dk ring-2 ring-inset ring-brand/20 transition hover:ring-brand/40"
                >
                  {photoUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={photoUrl} alt={`${prenom} ${nom}`} className="h-full w-full object-cover" />
                  ) : (
                    initials(`${prenom} ${nom}`.trim() || "?")
                  )}
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-muted-foreground">
                    ...
                    {String(profil?.matricule ?? (fallback?.etu as unknown as Record<string, string> | undefined)?.matricule ?? "")}
                  </p>
                  <div className="mt-1.5 flex flex-wrap gap-1.5">
                    <span className={toneBadge("teal")}>{String(profil?.filiere ?? fallback?.etu.filiere ?? "")}</span>
                    <span className={toneBadge("blue")}>{String(profil?.niveau ?? fallback?.etu.niveau ?? "")}</span>
                  </div>
                </div>
              </div>
              <DetailGrid>
                <DetailField label="Groupe" value={String(profil?.groupe ?? fallback?.etu.groupe ?? "—")} />
                <DetailField label="Année" value={String(profil?.annee ?? (fallback?.etu as unknown as Record<string, string> | undefined)?.annee ?? "—")} />
                <DetailField label="Téléphone" value={String(profil?.telephone ?? (fallback?.etu as unknown as Record<string, string> | undefined)?.telephone ?? "—")} />
                <DetailField label="Email" value={String(profil?.email ?? (fallback?.etu as unknown as Record<string, string> | undefined)?.email ?? "—")} />
                <DetailField label="Ville" value={String(profil?.ville ?? (fallback?.etu as unknown as Record<string, string> | undefined)?.ville ?? "—")} />
                <DetailField label="Naissance" value={String(profil?.dateNaissance ?? (profil as unknown as Record<string, string> | undefined)?.date_naissance ?? "—")} />
              </DetailGrid>
              <div className="flex flex-wrap gap-1.5">
                <span className={toneBadge("neutral")}>Statut : {String(profil?.statut ?? fallback?.etu.statut ?? "—")}</span>
                <span className={toneBadge("amber")}>Paiement : {String(profil?.paiement ?? fallback?.etu.paiement ?? "—")}</span>
              </div>
            </section>
            ...
            <section className={cn(softCard, "space-y-3 p-5")}>
              ...
              {enseignants.length ? (
                    ...
                    <li key={String(f.id ?? i)} className="flex items-center gap-3">
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="block truncate text-xs text-muted-foreground">
                          ...
                          {Array.isArray(f.modules) && f.modules.length ? ` · ${(f.modules as string[]).slice(0, 2).join(", ")}` : ""}
                        </span>
                      </span>
                    </li>
                  ))}
                ...
                <p className="text-sm text-muted-foreground">Aucun enseignant rattaché à votre groupe pour le moment.</p>
              )}
              <DetailSection title="Scolarité">
                <DetailGrid>
                  <DetailField label="Moyenne" value={notes.length ? `${(notes.reduce((s, n) => s + n.note, 0) / notes.length).toFixed(2)}/20` : "—"} />
                  <DetailField label="Présence" value={`${presence.taux}% (${presence.presents}/${presence.total})`} />
                  <DetailField label="Bulletins" value={String(bulletins.length)} />
                  <DetailField label="Paiements suivis" value={String(paiements.length)} />
                </DetailGrid>
              </DetailSection>
            </section>
            ...
            <section className={cn(softCard, "space-y-3 p-5")}>
              ...
              {stageEnCours ? (
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <p className="text-sm font-bold text-foreground">{String(stageEnCours.structure ?? "")}</p>
                    <span className={toneBadge("blue")}>{String(stageEnCours.statut ?? "")}</span>
                  </div>
                  <DetailGrid>
                    <DetailField label="Service" value={String(stageEnCours.service ?? "—")} />
                    <DetailField label="Période" value={`${String(stageEnCours.debut ?? "")} → ${String(stageEnCours.fin ?? "")}`} />
                    <DetailField label="Encadrant" value={String(stageEnCours.encadrantClinique ?? stageEnCours.encadrant_clinique ?? "—")} />
                    <DetailField label="Tuteur" value={String(stageEnCours.tuteurAcademique ?? stageEnCours.tuteur_academique ?? "—")} />
                  </DetailGrid>
                </div>
              ) : (
                <p className="text-sm text-muted-foreground">Aucun stage en cours. Le secrétariat vous affectera.</p>
              )}
              ...
              <DetailSection title="Documents">
                <p className="text-xs text-muted-foreground">
                  Convention, rapports et attestations sont délivrés par le secrétariat après traitement de vos demandes.
                </p>
              </DetailSection>
            </section>
          </div>
          ...
          <section className={cn(softCard, "space-y-3 p-4 sm:p-5")}>
            ...
            <div className="flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground">
              <button className={ghostPill} onClick={() => setAncre(new Date())}>Aujourd'hui</button>
              <span>
                Semaine du {joursSemaine[0].toLocaleDateString("fr-FR")} — {seances.length} séance(s) affichée(s)
              </span>
            </div>
          </section>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
                ...
                <ul className="divide-y divide-brand/8">
                  {notes.slice(0, 8).map((n, i) => (
                    <li key={i} className="flex items-center justify-between py-2 text-sm">
                      ...
                      <span className={cn("font-bold tabular-nums", n.note < 10 ? "text-alert" : "text-brand-dk")}>
            ...
            <section className={cn(softCard, "space-y-3 p-5")}>
              ...
              <DetailGrid>
                <DetailField label="Reste à payer" value={`${String(profil?.resteAPayer ?? profil?.reste_a_payer ?? "—")} MAD`} />
          ...
          <section className={cn(softCard, "space-y-3 p-4 sm:p-5")}>
            ...
            <DataTable
              ...
              empty="Aucune demande. Utilisez « Nouvelle demande » en haut à droite."
              head={
                <>
                  <th>Titre</th>
                  <th>Type</th>
            ...
            >
              {demandes.map((d) => (
                <tr key={d.id} className={tableRow}>
                  ...
                  <td className="text-muted-foreground">{d.type === "predefini" ? "Prédéfini" : "Libre"}</td>
                  <td>
                    <span className={toneBadge(STATUT_DEMANDE_TONE[d.statut])}>{STATUT_DEMANDE_LABEL[d.statut]}</span>
                  </td>
                  <td className="text-muted-foreground">{new Date(d.createdAt).toLocaleDateString("fr-FR")}</td>
          ...
          {isStaff ? (
            <section className={cn(softCard, "space-y-3 p-4 sm:p-5")}>
              <div className="flex items-center gap-2">
                ...
                <p className={eyebrowClass}>Demandes à traiter ({(allReqQuery.data ?? []).length})</p>
              </div>
              <DataTable
                ...
                {(allReqQuery.data ?? []).slice(0, 20).map((d) => (
                  <tr key={d.id} className={tableRow}>
                    <td className={cn("font-medium", cellTruncate)}>
                      {d.titre}
                      <span className="block truncate text-xs font-normal text-muted-foreground">{d.description}</span>
                    </td>
                    <td>
                      <span className={toneBadge(STATUT_DEMANDE_TONE[d.statut])}>{STATUT_DEMANDE_LABEL[d.statut]}</span>
                    </td>
