...
import { login, createUser, listAllUsers, updateUser, deleteUser, findByEmail } from "@/services/auth";
...
export async function authRoutes(app: FastifyInstance) {
  app.post("/login", async (request, reply) => {
    ...
    const token = app.jwt.sign({ id: user.id, email: user.email, name: user.name, role: user.role as "admin" | "superadmin" });
    return { token, user };
  });
